...
void align_prefix(BlockHandle block, PaddedBytes* a, PaddedBytes* a_3di, PaddedBytes* b, PaddedBytes* b_3di, const AAMatrix* matrix, const AAMatrix* matrix_3di, Gaps gaps, int32_t target_score, AlignResult* res, Cigar* cigar) {
    size_t min_size = 32;
    ...
    while (min_size <= MAX_SIZE && res->score < target_score) {
        ...
        block_align_3di_aa_trace_xdrop(block, a, a_3di, b, b_3di, matrix, matrix_3di, gaps, range, x_drop);
...
void example(void) {
    ...
    // note: instead of a_len or b_len, it is possible to use a really large length
    ...
    align_prefix(block, a, a_3di, b, b_3di, &BLOSUM62, matrix_3di, gaps, target_score, &res, cigar);
