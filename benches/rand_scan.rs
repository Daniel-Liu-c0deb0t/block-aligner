...
fn bench_scan_aa_core_insert<const K: usize>(b: &mut Bencher, len: usize) {
    let mut rng = StdRng::seed_from_u64(1234);
    let r = black_box(rand_str(len, &AMINO_ACIDS, &mut rng));
    let q = black_box(rand_mutate_insert(&r, K, &AMINO_ACIDS, len / 10, &mut rng));
    let r = PaddedBytes::from_bytes(&r, 256, false);
    let q = PaddedBytes::from_bytes(&q, 256, false);
    type BenchParams = GapParams<-11, -1>;
...
fn bench_scan_nuc_core<const K: usize>(b: &mut Bencher, len: usize) {
    ...
    b.iter(|| {
        let a = Block::<BenchParams, _, 16, 256, false, false>::align(&q, &r, &NW1, 0, 10, 0);
        a.res()
    });
}
...
#[bench]
fn bench_scan_aa_10_100_insert(b: &mut Bencher) { bench_scan_aa_core_insert::<10>(b, 100); }
#[bench]
fn bench_scan_aa_100_1000_insert(b: &mut Bencher) { bench_scan_aa_core_insert::<100>(b, 1000); }
#[bench]
fn bench_scan_aa_1000_10000_insert(b: &mut Bencher) { bench_scan_aa_core_insert::<1000>(b, 10000); }
