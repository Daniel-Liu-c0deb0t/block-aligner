...
fn bench_scan_aa_core<const K: usize>(b: &mut Bencher, len: usize, insert: bool) {
    ...
    b.iter(|| {
        let a = Block::<BenchParams, _, 32, 2048, false, false>::align(&q, &r, &BLOSUM62, 0);
        a.res()
    });
}
...
fn bench_scan_aa_core_small<const K: usize>(b: &mut Bencher, len: usize) {
    ...
    b.iter(|| {
        let a = Block::<BenchParams, _, 16, 16, false, false>::align(&q, &r, &BLOSUM62, 0);
        a.res()
    });
}
...
fn bench_scan_aa_core_trace<const K: usize>(b: &mut Bencher, len: usize) {
    ...
    b.iter(|| {
        let a = Block::<BenchParams, _, 32, 2048, true, false>::align(&q, &r, &BLOSUM62, 0);
        //a.res()
